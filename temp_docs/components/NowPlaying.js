...
const NowPlaying = ({ isOpen, onClose, station, isPlaying, onPlayPause, onNext, onPrev, volume, onVolumeChange, trackInfo, showNextSong, frequencyData, visualizerStyle, isVisualizerEnabled, onCycleVisualizerStyle, isVolumeControlVisible, marqueeDelay, isMarqueeProgramEnabled, isMarqueeCurrentTrackEnabled, isMarqueeNextTrackEnabled, marqueeSpeed, onOpenActionMenu, isVisualizerFullscreen, setIsVisualizerFullscreen }) => {
    ...
    return (React.createElement("div", { ref: dragRef, className: `fixed bg-bg-primary z-50 flex flex-col h-full transition-transform duration-300 ease-in-out ${isOpen ? 'translate-y-0' : 'translate-y-full'} ${isVisualizerFullscreen ? 'inset-0' : 'inset-x-0 bottom-0'}`, onTouchStart: handleTouchStart, onTouchMove: handleTouchMove, onTouchEnd: handleTouchEnd },
        ...
        React.createElement("div", { ref: scrollRef, className: `flex-grow flex flex-col items-center justify-center gap-4 text-center overflow-y-auto py-4 px-4 ${isVisualizerFullscreen ? 'h-full' : ''}` },
            React.createElement("img", { src: station?.favicon || 'https://picsum.photos/256', alt: station?.name || 'תחנה', className: `rounded-2xl bg-gray-700 object-cover shadow-2xl flex-shrink-0 transition-all duration-300 ${isVisualizerFullscreen ? 'hidden' : 'w-48 h-48 sm:w-56 sm:h-56 md:w-64 md:h-64'}`, onError: (e) => { e.currentTarget.src = 'https://picsum.photos/256'; } }),
            React.createElement("div", { className: `flex-shrink-0 w-full ${isVisualizerFullscreen ? 'hidden' : ''}`, key: station?.stationuuid },
                React.createElement("div", { className: "w-full px-4" },
                    React.createElement(MarqueeText, { loopDelay: marqueeDelay, duration: marqueeConfig.duration, startAnimation: startAnimation, isOverflowing: marqueeConfig.isOverflowing[0] && isMarqueeProgramEnabled, contentRef: stationNameRef, className: "text-2xl sm:text-3xl font-bold text-text-primary" },
                ...
                React.createElement("div", { className: "mt-2 min-h-[4rem] flex flex-col justify-center items-center" },
                    React.createElement("div", { className: "w-full px-4 text-center" },
                        trackInfo?.program && !trackInfo.current && (React.createElement(MarqueeText, { loopDelay: marqueeDelay, duration: marqueeConfig.duration, startAnimation: startAnimation, isOverflowing: marqueeConfig.isOverflowing[1] && isMarqueeProgramEnabled, contentRef: programNameRef, className: "text-lg text-text-primary opacity-80" },
                            React.createElement("span", null, trackInfo.program))),
                        trackInfo?.current && (React.createElement(React.Fragment, null,
                            trackInfo.program && (React.createElement(MarqueeText, { loopDelay: marqueeDelay, duration: marqueeConfig.duration, startAnimation: startAnimation, isOverflowing: marqueeConfig.isOverflowing[1] && isMarqueeProgramEnabled, contentRef: programNameRef, className: "text-base text-text-primary opacity-70" },
                                React.createElement("span", null, trackInfo.program))),
                            React.createElement("div", { className: "mt-1" },
                                React.createElement(MarqueeText, { loopDelay: marqueeDelay, duration: marqueeConfig.duration, startAnimation: startAnimation, isOverflowing: marqueeConfig.isOverflowing[2] && isMarqueeCurrentTrackEnabled, contentRef: currentTrackRef },
                                    React.createElement(InteractiveText, { text: trackInfo.current, className: "font-bold text-xl", onOpenActionMenu: onOpenActionMenu })))))),
                    showNextSong && trackInfo?.next && (React.createElement("div", { className: "w-full px-4 mt-2 flex items-center justify-center text-base text-text-secondary opacity-90" },
                        React.createElement("span", { className: "font-semibold flex-shrink-0" }, "\u05D4\u05D1\u05D0:\u00A0"),
                        React.createElement("div", { className: "min-w-0" },
                            React.createElement(MarqueeText, { loopDelay: marqueeDelay, duration: marqueeConfig.duration, startAnimation: startAnimation, isOverflowing: marqueeConfig.isOverflowing[3] && isMarqueeNextTrackEnabled, contentRef: nextTrackRef },
                                React.createElement("span", null, trackInfo.next))))))),
            React.createElement("div", { className: `w-full max-w-sm px-4 flex-shrink-0 visualizer-interaction-area ${isVisualizerFullscreen ? 'w-full h-full max-w-none' : 'h-20'}` }, isVisualizerEnabled && (React.createElement(Visualizer, { frequencyData: frequencyData, style: visualizerStyle })))),
        React.createElement("div", { className: `flex-shrink-0 flex flex-col items-center gap-4 sm:gap-6 pb-4 sm:pb-8 px-4 ${isVisualizerFullscreen ? 'hidden' : ''}` },
            React.createElement("div", { className: "flex items-center justify-center gap-4" },
                ...
                React.createElement("button", { onClick: onPlayPause, className: "p-5 bg-accent text-white rounded-full shadow-lg hover:bg-accent-hover transition-transform transform hover:scale-105", "aria-label": isPlaying ? "השהה" : "נגן" }, isPlaying ? React.createElement(PauseIcon, { className: "w-14 h-14" }) : React.createElement(PlayIcon, { className: "w-14 h-14" })),
            ...
            isVolumeControlVisible && (React.createElement("div", { className: "w-full max-w-xs flex items-center gap-3" },
                React.createElement(VolumeUpIcon, { className: "w-6 h-6 text-text-secondary flex-shrink-0" }),
                React.createElement("input", { type: "range", min: "0", max: "1", step: "0.01", value: volume, onChange: (e) => onVolumeChange(parseFloat(e.target.value)), onWheel: (e) => {
                    ...
                    }, className: "w-full accent-teal-500", "aria-label": "\u05E2\u05D5\u05E6\u05DE\u05EA \u05E9\u05DE\u05E2" }))))));
};
