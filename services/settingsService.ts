...
import { AllSettings, EqPreset, CustomEqSettings, VisualizerStyle, GridSize, StationFilter, SortOrder, KeyMap, SettingsSections } from '../types/settings';
...
export const fetchCustomStations = async (): Promise<Station[] | null> => {
    try {
        ...
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
            return docSnap.data().stations as Station[];
        }
        return null;
    } catch (error) {
...
export const normalizeSettings = (raw: any): AllSettings => {
...
export const settingsHaveConflict = (local: AllSettings, cloud: AllSettings): boolean => {
