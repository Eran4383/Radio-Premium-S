...
export const STATION_SCHEDULES: StationSchedule[] = [
  {
    name: 'כאן ב',
  ...
  },
  {
    ...
    schedule: [
      ...[0, 1, 2, 3, 4].flatMap(day => [
        ...
        { day, start: "16:00", end: "18:00", name: "ארבע אחרי הظهر עם יואב צפיר" },
...
export const getCurrentProgram = (stationName: string): string | null => {
  ...
  const station = STATION_SCHEDULES.find(s => s.aliases.some(a => low.includes(a.toLowerCase())));
  if (!station) return null;

  try {
    const formatter = new Intl.DateTimeFormat('en-US', {
      timeZone: 'Asia/Jerusalem', hour12: false, hour: '2-digit', minute: '2-digit', weekday: 'long',
    });
    const parts = formatter.formatToParts(new Date());
    const getPart = (type: string) => parts.find(p => p.type === type)?.value || '';
    const day = weekdayMap[getPart('weekday').toLowerCase()];
    const currentTime = `${getPart('hour')}:${getPart('minute')}`;

    const current = station.schedule.filter(p => p.day === day).find(p => {
      if (p.start > p.end) return currentTime >= p.start || currentTime < p.end;
