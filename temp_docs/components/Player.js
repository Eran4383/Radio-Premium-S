    ...
    return (React.createElement("div", { className: "fixed bottom-0 left-0 right-0 z-30" },
        React.createElement("div", { className: "relative bg-bg-secondary/80 backdrop-blur-lg shadow-t-lg" },
            ...
            React.createElement("div", { className: "max-w-7xl mx-auto p-4 flex items-center justify-between gap-4" },
                React.createElement(PlayerInfo, { station: station, trackInfo: trackInfo, status: status, error: error, showNextSong: showNextSong, onOpenNowPlaying: onOpenNowPlaying, onOpenActionMenu: onOpenActionMenu, isMarqueeProgramEnabled: isMarqueeProgramEnabled, isMarqueeCurrentTrackEnabled: isMarqueeCurrentTrackEnabled, isMarqueeNextTrackEnabled: isMarqueeNextTrackEnabled, marqueeSpeed: marqueeSpeed, marqueeDelay: marqueeDelay, isSmartPlayerActive: !!isSmartPlayerActive }),
                React.createElement(PlayerControls, { isActuallyPlaying: isPlaying, isLoading: isLoading, onPlayPause: onPlayPause, onSmartPrev: handleSmartPrev, onSmartNext: handleSmartNext })),
            React.createElement("audio", { ref: audioRef, onPlaying: () => onPlayerEvent({ type: 'STREAM_STARTED' }), onPause: () => onPlayerEvent({ type: 'STREAM_PAUSED' }), onTimeUpdate: () => { }, onStalled: () => attemptRecovery(), onWaiting: () => { }, onError: () => onPlayerEvent({ type: 'STREAM_ERROR', payload: "שגיאה בניגון התחנה." }) }))));
};
