...
const fetchGaleiTzahalScheduleInfo = async () => {
    try {
        ...
        const data = JSON.parse(await res.text());
        const today = data?.timeTable?.glzTimeTable?.find((day) => day.isToday);
        const now = new Date();
        const current = today?.programmes?.find((p) => now >= new Date(p.start) && now < new Date(p.end));
...
export const fetchStationSpecificTrackInfo = async (stationName) => {
    const low = stationName.toLowerCase();
    if (Object.keys(GLZ_SLUGS).some(k => low.includes(k.toLowerCase())))
        return fetchGaleiTzahalCombinedInfo(stationName);
    if (Object.keys(KAN_STATION_IDS).some(k => low.includes(k.toLowerCase())))
        return fetchKanTrackInfo(stationName);
    if (low.includes('eco99fm') || low.includes('99fm'))
